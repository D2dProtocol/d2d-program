...
pub fn close_treasury_pool(ctx: Context<CloseTreasuryPool>) -> Result<()> {
  ...
  msg!(
    "[CLOSE] Account balance before close: {} lamports",
    balance_before
  );
  ...
  if balance_before <= rent_exempt_minimum {
    ...
    msg!(
      "[CLOSE] Balance: {} lamports, Rent minimum: {} lamports",
      balance_before,
      rent_exempt_minimum
    );
  }
  ...
  if transfer_amount > 0 {
    msg!(
      "[CLOSE] Transferring {} lamports to verified admin",
      transfer_amount
    );
  ...
  msg!(
    "[CLOSE] Remaining balance: {} lamports (rent-exempt minimum)",
