...
pub struct ProxyUpgradeProgram<'info> {
    ...
    /// Deploy request - validates subscription status
    #[account(
...
pub fn proxy_upgrade_program(ctx: Context<ProxyUpgradeProgram>) -> Result<()> {
    ...
    // This is required because BPF Loader's upgrade instruction requires 
    // the buffer authority to match the program's upgrade authority (our PDA).
    // Since the developer is a signer, we can CPI to set_buffer_authority.
