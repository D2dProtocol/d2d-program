...
impl DeveloperEscrow {
    ...
    pub fn deduct_balance(&mut self, amount: u64, token_type: TokenType) -> Result<()> {
        match token_type {
            TokenType::SOL => {
                require!(self.sol_balance >= amount, ErrorCode::InsufficientEscrowBalance);
                self.sol_balance = self.sol_balance.checked_sub(amount)
                    .ok_or(ErrorCode::CalculationOverflow)?;
            }
            TokenType::USDC => {
                require!(self.usdc_balance >= amount, ErrorCode::InsufficientEscrowBalance);
                self.usdc_balance = self.usdc_balance.checked_sub(amount)
                    .ok_or(ErrorCode::CalculationOverflow)?;
            }
            TokenType::USDT => {
                require!(self.usdt_balance >= amount, ErrorCode::InsufficientEscrowBalance);
                self.usdt_balance = self.usdt_balance.checked_sub(amount)
                    .ok_or(ErrorCode::CalculationOverflow)?;
            }
        }

        self.total_auto_deducted = self.total_auto_deducted.checked_add(amount)
            .ok_or(ErrorCode::CalculationOverflow)?;
        ...
        match token_type {
            TokenType::SOL => {
                self.sol_balance = self.sol_balance.checked_add(amount)
                    .ok_or(ErrorCode::CalculationOverflow)?;
                self.total_deposited_sol = self.total_deposited_sol.checked_add(amount)
                    .ok_or(ErrorCode::CalculationOverflow)?;
            }
            TokenType::USDC => {
                self.usdc_balance = self.usdc_balance.checked_add(amount)
                    .ok_or(ErrorCode::CalculationOverflow)?;
                self.total_deposited_usdc = self.total_deposited_usdc.checked_add(amount)
                    .ok_or(ErrorCode::CalculationOverflow)?;
            }
            TokenType::USDT => {
                self.usdt_balance = self.usdt_balance.checked_add(amount)
                    .ok_or(ErrorCode::CalculationOverflow)?;
                self.total_deposited_usdt = self.total_deposited_usdt.checked_add(amount)
                    .ok_or(ErrorCode::CalculationOverflow)?;
            }
        }
